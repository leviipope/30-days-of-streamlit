...
# create dataframes form the functions
df_agg, df_agg_sub, df_comments, df_time = load_data()

# engineer data
df_agg_diff = df_agg.copy() # so it doesnt mess up anything in df_agg, its just good practice
...
if add_sidebar == 'Aggregate Metrics':
    ...
    for i in metric_medians6mo.index:
        if i == 'Video publish time':
            continue
        with columns[count]:
            if isinstance(metric_medians6mo[i], (pd.Timestamp, datetime)):
                st.metric(label= i, value = metric_medians6mo[i].strftime('%Y-%m-%d'))
            else:
